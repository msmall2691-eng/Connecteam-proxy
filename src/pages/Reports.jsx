...
export default function Reports() {
  ...
  async function pullData(numWeeks) {
    ...
    setError(null)
    try {
      const { start, end } = dateRangeWeeks(numWeeks || weeks)
  ...
  function buildDataContext() {
    ...
    for (const u of actUsers) {
      ...
      for (const s of userShifts) {
        const start = s.startTime ? new Date(s.startTime * 1000) : null
        const end = s.endTime ? new Date(s.endTime * 1000) : null
        const hours = start && end ? ((s.endTime - s.startTime) / 3600).toFixed(1) : '?'
        let miles = 0
        for (const att of s.shiftAttachments || []) {
          if (att.attachment?.number) miles += att.attachment.number
        }
        const loc = s.startPoint?.address || 'no location'
        const notes = [s.startNote, s.endNote].filter(Boolean).join(' | ')
        lines.push(`  ${start?.toLocaleDateString() || '?'} ${start?.toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'}) || ''}-${end?.toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'}) || ''} | ${hours}h | ${miles}mi | ${loc}${notes ? ` | Notes: ${notes}` : ''}`)
      }
    }
      ...
      {step === 'choose' && (
        <>
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 space-y-4">
            ...
            />
            <button onClick={() => pullData()} disabled={!customPrompt.trim()}
